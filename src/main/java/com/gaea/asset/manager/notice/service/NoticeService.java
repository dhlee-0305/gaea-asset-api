...
public class NoticeService {
	...
	public Header<List<NoticeVO>> getNoticeList(int currentPage, int pageSize, Search search) {
        try {
            ...
            if (boardList == null || boardList.isEmpty()) {
                return Header.OK(ResultCode.NO_CONTENT, "등록된 공지사항이 없습니다.", boardList);
            }
            return Header.OK(boardList, pagination);
        } catch (Exception e) {
            return Header.ERROR(ResultCode.INTERNAL_SERVER_ERROR, "공지사항 목록 조회 중 오류가 발생했습니다.");
        }
	...
	public Header<NoticeVO> getNoticeInfo(Long noticeNum) {
        if (noticeNum == null) {
            return Header.ERROR(ResultCode.BAD_REQUEST, "공지사항 번호가 누락되었습니다.");
        }
        try {
            NoticeVO noticeVO = noticeMapper.getNoticeInfo(noticeNum);
            if (noticeVO == null) {
                return Header.OK(ResultCode.NO_CONTENT, "해당 공지사항을 찾을 수 없습니다.", null);
            }
            ...
            return Header.ERROR(ResultCode.INTERNAL_SERVER_ERROR, "공지사항 상세 조회 중 오류가 발생했습니다.");
        }
	}
    ...
    public Header<NoticeVO> insertNotice(NoticeVO noticeVO, List<MultipartFile> files) {
        try {
            ...
            if (result <= 0) {
                return Header.ERROR(ResultCode.BAD_REQUEST, "공지사항 등록에 실패했습니다.");
            }
            Long noticeNum = noticeVO.getNoticeNum();
            fileService.saveFile(files, noticeNum, CommonCode.POST_TYPE_NOTICE);
            return Header.OK(ResultCode.OK, "공지사항이 등록되었습니다.", noticeVO);
        } catch (IllegalArgumentException e) {
            TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
            return Header.ERROR(ResultCode.BAD_REQUEST, e.getMessage());
        } catch (IOException e) {
            TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
            return Header.ERROR(ResultCode.INTERNAL_SERVER_ERROR, "파일 저장 중 오류가 발생했습니다.");
        } catch (Exception e) {
            TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
            return Header.ERROR(ResultCode.INTERNAL_SERVER_ERROR, "공지사항 등록 중 오류가 발생했습니다.");
        }
    }
	...
	public Header<NoticeVO> updateNotice(NoticeVO noticeVO, List<MultipartFile> files) {
        try {
            ...
            if (result <= 0) {
                return Header.ERROR(ResultCode.BAD_REQUEST, "공지사항 수정에 실패했습니다.");
            }
            Long noticeNum = noticeVO.getNoticeNum();
            fileService.saveFile(files, noticeNum, CommonCode.POST_TYPE_NOTICE);
            return Header.OK(ResultCode.OK, "공지사항이 수정되었습니다.", noticeVO);
        } catch (IllegalArgumentException e) {
            TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
            return Header.ERROR(ResultCode.BAD_REQUEST, e.getMessage());
        } catch (IOException e) {
            ...
            return Header.ERROR(ResultCode.INTERNAL_SERVER_ERROR, "공지사항 수정 중 오류가 발생했습니다." + e.getMessage());
        }
	}
	...
	public Header<String> deleteNotice(Long noticeNum) {
        try {
            List<FileVO> fileList = fileService.getFileList(noticeNum);
            String savePath = fileService.savePath(CommonCode.POST_TYPE_NOTICE);
            ...
            fileService.deleteFile(CommonCode.POST_TYPE_NOTICE, noticeNum);
            if (noticeMapper.deleteNotice(noticeNum) > 0) {
                return Header.OK(ResultCode.OK, "공지사항이 삭제되었습니다.", String.valueOf(noticeNum));
            } else {
                return Header.ERROR(ResultCode.BAD_REQUEST, "공지사항 삭제에 실패했습니다.");
            }
        } catch (Exception e) {
            return Header.ERROR(ResultCode.INTERNAL_SERVER_ERROR, "공지사항 삭제 중 오류가 발생했습니다.");
        }
    }
}
